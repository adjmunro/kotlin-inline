...
/**
 * Syntax-sugar for a lambda that folds a receiver type [In] into type [Out].
 *
 ...
 * ```
 *
 * > Named `boolfold` to avoid confusion with the `fold` function in Kotlin's standard library,
 * > this type of fold is akin to a monadic fold operation that transforms the backing field
 * > into *a new type* for all wrapper states.
 *
 ...
 */
@InlineDsl
public inline fun <In, Out> In.boolfold(
 ...
 * ```
 *
 * > Similar to [boolfold], `flatmap` is akin to a monadic flatmap operation that transforms
 * > the backing field into *the same type* for all wrapper states.
 *
...
public inline fun <T> T.flatmap(
    ...
    falsy: T.() -> T = ::caller,
    truthy: T.() -> T = ::caller,
): T = boolfold(predicate = predicate, falsy = falsy, truthy = truthy)
